...
/*		Part part = request.getPart("file");
		...
		//String fileName =root+"/"+Calendar.getInstance().get(Calendar.SECOND)+(counter++)+"temp"+".jpg";
		ServletContext application = this.getServletContext();
		int counter =(int) application.getAttribute("counter");
		counter=(++counter)%100;
		application.setAttribute("counter", counter);
		String fileName2="temp/"+counter+"temp.jpg";
		//part.write(filename);
	...
	//	Mat image = Highgui.imread("c:\\temp\\upload\\temp.jpg");
		...
		int flag = 0;
		try {
			db.res=db.pst.executeQuery();
			while(db.res.next()){
				if(new TiQu().tiqu(image,db.res.getString(3))<1100){
					request.setAttribute("name",db.res.getString(1) );
		...
		if(flag == 0){
				request.setAttribute("name","识别失败，数据库尚无记录" );
			}
